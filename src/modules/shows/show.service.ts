...
import { IAggregatedCreditGroupQueryResponse, ITheOpenMovieDatabaseShow } from './types';
import { Nullable } from '../../common/types/Nullable';
import { EntertainmentService } from '../../core/entertainment/entertainment.service';
import { CurrentSeason, ENTERTAINMENT_TYPES } from '../../graphql/generated/schema';
...
export class ShowService {
	...
	async getCastAggregatedCreditGroup(showId: number) {
		const { data } = await firstValueFrom(
			this.httpService.get<IAggregatedCreditGroupQueryResponse>(
				`https://api.themoviedb.org/3/tv/${showId}/aggregate_CreditGroup?language=en-U`,
				{
					headers: {
						Accept: 'application/json',
						Authorization: `Bearer ${this.configService.get('THE_OPEN_MOVIE_DATABASE_API_KEY')}`
	...
	async getTopBilledCast(showId: number) {
		...
		const aggregatedCreditGroup = await this.getCastAggregatedCreditGroup(showId);

		return topBilledCast.map((topBilledCastMember) => {
			// Find the cast members CreditGroup data, found via the aggregate_CreditGroup endpoint
			const aggregatedCredit = aggregatedCreditGroup.cast.find(
